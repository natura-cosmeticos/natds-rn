...
import {
  Wrapper, InputWrapper, TextInput, Label, AssistiveTexView, InputIcon, IconPress,
...
export const NatTextInput = ({
  ...
  const renderIcon = (iconData) => {
    console.log('renderIcon', iconData);

    if (iconData.svg) {
      const IconComponent = loadSvg(iconData.svg.name);
      const iconSize = (iconData.svg.size) ? iconData.svg.size : 24;
      const iconColor = (iconData.svg.color) ? iconData.svg.color : ThemeNaturaLight.palette.text.primary;
      console.log('IconComponent', IconComponent, iconSize);
      return (
