...
export const ButtonWrapper = styled.TouchableOpacity`
  background-color: ${({ type, color, theme }) =>
    styleType(type, color, theme).backgroundColor};
  height: 48;
  border-radius: 4;
  align-items: center;
  justify-content: center;
  flex-direction: row;
  border-color: ${({ type, color, theme }) =>
    styleType(type, color, theme).borderColor};
  border-width: ${({ type, color, theme }) =>
    styleType(type, color, theme).borderWidth};
`;
