...
export const loadIcon = (icon, attributes) => {
  ...
  if (icon && icon.content) {
    return (
      <Image style={{ height: attributes.size, tintColor: attributes.color, width: attributes.size }} source={icon.content} />
    );
  }
  if (icon && icon.uri) {
    return (
      <Image style={{ height: attributes.size, width: attributes.size }} source={icon} />
    );
  }
